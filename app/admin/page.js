...
import { Trash2, RotateCcw, AlertTriangle, CheckCircle, XCircle, Loader, Plus, X, ChevronDown, ChevronUp } from 'lucide-react'

// ─── Constants ────────────────────────────────────────────────────────────────

const CATEGORIES = ['Lunch', 'Dinner', 'Dessert', 'Breakfast & Brunch', 'Drinks & Shakes', 'Snacks & Sides']
const CUISINES = ['American', 'Italian', 'French', 'Mexican', 'Asian', 'Mediterranean', 'Middle Eastern', 'Other']
...
const EMPTY_FORM = {
  title: '', shortDescription: '', slug: '', videoUrl: '',
  category: '', cuisine: '', tags: [], featured: false,
  publishedAt: new Date().toISOString().slice(0, 10),
  difficulty: '', prepTime: '', cookTime: '', servings: '', caloriesPerServing: '',
  ingredients: [''],
  steps: [{ title: '', text: '' }],
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          <p style={{ fontSize: '0.72rem', letterSpacing: '2px', textTransform: 'uppercase', color: '#E8622A', marginBottom: '0.5rem', fontWeight: '700' }}>
            localhost only
          </p>
          <h1 style={{ fontFamily: '"Playfair Display", serif', fontSize: '2.2rem', color: '#FDF6EE', margin: 0 }}>
            Admin Panel
          </h1>
...
function NewRecipeTab() {
  ...
  const handleSubmit = async () => {
    ...
    setStatus(null)
    try {
      const res = await fetch('/api/admin/create-recipe', {
        method: 'POST',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {['form', 'json'].map(mode => (
          <div
            ...
            style={{
              ...
              userSelect: 'none',
              background: inputMode === mode ? 'rgba(232,98,42,0.2)' : 'rgba(255,255,255,0.04)',
              color: inputMode === mode ? '#F4946A' : 'rgba(253,246,238,0.5)',
              border: inputMode === mode ? '1px solid rgba(232,98,42,0.4)' : '1px solid rgba(255,255,255,0.08)',
            }}
          >
            {mode === 'form' ? 'Form' : 'JSON'}
          </div>
      ...
      {inputMode === 'json' ? (
        <Card>
          ...
          <textarea
            ...
            style={{
              ...
              border: `1px solid ${jsonError ? 'rgba(220,53,69,0.5)' : 'rgba(255,255,255,0.12)'}`,
              borderRadius: '12px', padding: '1rem', color: '#FDF6EE',
              fontFamily: 'monospace', fontSize: '0.82rem', resize: 'vertical',
              outlineColor: '#E8622A', boxSizing: 'border-box',
            }}
          />
        ...
        <>
          ...
          <Section title="Basic Info">
            <Row>
              <Field label="Recipe Title *">
                <Input value={form.title} onChange={e => handleTitleChange(e.target.value)} placeholder="Creamy Tomato Pasta" />
              </Field>
              <Field label="Slug *">
            ...
            <Field label="Short Hook / Description">
              <Textarea value={form.shortDescription} onChange={e => set('shortDescription', e.target.value)} placeholder="The creamiest weeknight pasta you'll ever make..." rows={2} />
            </Field>
            <Row>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '0.35rem' }}>
                  <div
                    ...
                    style={{
                      ...
                      background: form.featured ? '#E8622A' : 'rgba(255,255,255,0.12)',
                  ...
                  <span style={{ color: 'rgba(253,246,238,0.7)', fontSize: '0.88rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {ALL_TAGS.map(tag => (
                <div
                  ...
                  style={{
                    padding: '0.3rem 0.85rem', borderRadius: '50px', cursor: 'pointer',
                    fontFamily: '"Lato", sans-serif', fontSize: '0.78rem', fontWeight: '700',
                    userSelect: 'none', transition: 'all 0.12s',
                    background: form.tags.includes(tag) ? 'rgba(232,98,42,0.2)' : 'rgba(255,255,255,0.04)',
                    color: form.tags.includes(tag) ? '#F4946A' : 'rgba(253,246,238,0.5)',
                    border: form.tags.includes(tag) ? '1px solid rgba(232,98,42,0.5)' : '1px solid rgba(255,255,255,0.08)',
                  }}
                >
                  {tag}
                </div>
          ...
          {/* ── Media ── */}
          <Section title="Media">
            <Field label="Recipe Video (YouTube or TikTok URL)">
          ...
          <Section title="Storage Tips">
            {form.storageTips.map((s, i) => (
              <ListRow key={i} onRemove={() => removeItem('storageTips', i)}>
                ...
                <Input value={s.duration} onChange={e => updateItem('storageTips', i, { ...s, duration: e.target.value })} placeholder="Duration (e.g. 3 days)" />
  ...
  return (
    <Card>
      <div
        ...
        style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', userSelect: 'none', marginBottom: open ? '1rem' : 0 }}
      >
...
function Field({ label, children, style }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', flex: 1, ...style }}>
      <Label>{label}</Label>
      {children}
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: column ? 'column' : 'row', gap: '0.5rem', alignItems: column ? 'stretch' : 'center', marginBottom: '0.5rem' }}>
      ...
      <div
        ...
        style={{ color: 'rgba(220,53,69,0.7)', cursor: 'pointer', flexShrink: 0, padding: '0.25rem', alignSelf: column ? 'flex-end' : 'center' }}
      >
        <X size={14} />
      </div>
    </div>
...
function Input({ style, ...props }) {
  return (
    <input {...props} style={{
      ...
      borderRadius: '8px', padding: '0.55rem 0.85rem', color: '#FDF6EE',
      fontFamily: '"Lato", sans-serif', fontSize: '0.88rem', width: '100%',
      outlineColor: '#E8622A', boxSizing: 'border-box', ...style,
    }} />
  )
}
...
function Textarea({ ...props }) {
  return (
    <textarea {...props} style={{
      ...
      borderRadius: '8px', padding: '0.55rem 0.85rem', color: '#FDF6EE',
      fontFamily: '"Lato", sans-serif', fontSize: '0.88rem', width: '100%',
      outlineColor: '#E8622A', resize: 'vertical', boxSizing: 'border-box',
    }} />
  )
}
...
function Select({ children, ...props }) {
  return (
    <select {...props} style={{
      background: '#2A1208', border: '1px solid rgba(255,255,255,0.12)',
      borderRadius: '8px', padding: '0.55rem 0.85rem', color: '#FDF6EE',
      fontFamily: '"Lato", sans-serif', fontSize: '0.88rem', width: '100%',
      outlineColor: '#E8622A', boxSizing: 'border-box',
    }}>
  ...
  return (
    <div
      ...
      style={{
        ...
        color: '#F4946A', fontSize: '0.82rem', fontWeight: '700',
...
function StatusMessage({ status }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: '10px', color: isError ? '#ff6b7a' : '#6bcb77',
    ...
    <Card>
      ...
      <div style={{ position: 'relative' }}>
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          ...
          />
          <ActionButton onClick={handleSearch} loading={loading} disabled={!title.trim()}><Trash2/> Delete</ActionButton>
        </div>
          ...
          <div style={{ position: 'absolute', top: 'calc(100% + 0.5rem)', left: 0, right: 0, background: '#2A1208', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '14px', padding: '0.5rem', boxShadow: '0 12px 36px rgba(0,0,0,0.4)', zIndex: 10 }}>
            {suggestions.map(item => (
              <div key={item._id} role="button" tabIndex={0}
                ...
                onKeyDown={e => (e.key === 'Enter' || e.key === ' ') && (setTitle(item.title), setSuggestionsOpen(false), setSuggestions([]))}
              ...
              >
                ...
                <span><span style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title}</span></span>
              </div>
            ))}
          </div>
        )}
      ...
      {confirm && (
        <div style={{ marginTop: '1rem', padding: '1rem', background: 'rgba(220,53,69,0.08)', border: '1px solid rgba(220,53,69,0.25)', borderRadius: '12px' }}>
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.6rem' }}>
            ...
            <span style={{ color: '#ff6b7a', fontSize: '0.85rem', fontWeight: '700' }}>Found {confirm.length} recipe(s) — confirm deletion</span>
          </div>
    ...
    <Card>
      ...
      {step === 'confirm1' && (
        <div style={{ padding: '1rem', background: 'rgba(220,53,69,0.08)', border: '1px solid rgba(220,53,69,0.25)', borderRadius: '12px' }}>
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.75rem' }}>
            <AlertTriangle size={15} color="#ff6b7a" />
            <span style={{ color: '#ff6b7a', fontSize: '0.85rem', fontWeight: '700' }}>This cannot be undone. Are you sure?</span>
          </div>
      ...
      {step === 'confirm2' && (
        <div style={{ padding: '1rem', background: 'rgba(220,53,69,0.08)', border: '1px solid rgba(220,53,69,0.25)', borderRadius: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <input type="text" value={inputVal} onChange={e => setInputVal(e.target.value)} placeholder="DELETE ALL"
              style={{ flex: 1, minWidth: '150px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(220,53,69,0.4)', borderRadius: '50px', padding: '0.6rem 1.1rem', color: '#FDF6EE', fontFamily: '"Lato", sans-serif', fontSize: '0.9rem', outlineColor: '#dc3545' }}
            />
...
function ResetRatingsCard() {
  ...
  return (
    <Card>
      ...
      {!confirm ? (
        <ActionButton onClick={() => { setConfirm(true); setStatus(null) }}>Reset ratings</ActionButton>
      ) : (
        <div style={{ padding: '1rem', background: 'rgba(232,98,42,0.08)', border: '1px solid rgba(232,98,42,0.25)', borderRadius: '12px' }}>
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.75rem' }}>
            ...
            <span style={{ color: '#F4946A', fontSize: '0.85rem', fontWeight: '700' }}>All ratings will be wiped. Confirm?</span>
          </div>
          <div style={{ display: 'flex', gap: '0.75rem' }}>
